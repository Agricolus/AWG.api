...
{
    public class
    MeasuresQueryHandler
    :
    IRequestHandler<GetLastMeasure, MeasureDetail>,
    ...
    IRequestHandler<GetMonthlyMeasures, IEnumerable<MonthlyMeasureDetail>>
    {
        ...
        public async Task<MeasureDetail>
        Handle(GetLastMeasure request, CancellationToken cancellationToken)
        {
            ...
            select m)
                .Take(1)
                .ProjectTo<MeasureDetail>(mapper.ConfigurationProvider)
                .FirstOrDefaultAsync();
        }

        public async Task<IEnumerable<MeasureDetail>>
        Handle(GetMeasuresList request, CancellationToken cancellationToken)
        {
            return await(from m in db.WeatherObserved
            where
            m.DateObserved >= request.FromDate &&
            m.DateObserved < request.ToDate &&
            m.RefDevice == request.StationId
            select m)
                .OrderBy(f => f.DateObserved)
                .ProjectTo<MeasureDetail>(mapper.ConfigurationProvider)
                .ToListAsync();
        }

        public async Task<IEnumerable<DailyMeasureDetail>>
        Handle(GetDailyMeasures request, CancellationToken cancellationToken)
        {
            ...
            var d =
                await(from m in db.WeatherObserved
                where
                m.DateObserved >= fromdate &&
                (todate == null || m.DateObserved < todate) &&
                m.RefDevice == request.StationId
                group m by new { m.RefDevice, Date = m.DateObserved.Date }
                into
                gg
                select
                new DailyMeasureDetail()
                {
                    ...
                    AvgAtmosphericPressure =
                        gg.Average(g => g.AtmosphericPressure),
        ...
        public async Task<IEnumerable<WeeklyMeasureDetail>>
        Handle(GetWeeklyMeasures request, CancellationToken cancellationToken)
        {
            var fromdate = request.FromDate.Date;
            DateTime? todate = null;
            ...
            new {
                m.RefDevice,
                Year = m.DateObserved.Year,
                Week = 1 + (m.DateObserved.DayOfYear - 1) / 7
            }
            into
            gg
            select
            new WeeklyMeasureDetail()
            {
                StationId = gg.Key.RefDevice,
                Year = gg.Key.Year,
                Week = gg.Key.Week,
        ...
        public async Task<IEnumerable<MonthlyMeasureDetail>>
        Handle(GetMonthlyMeasures request, CancellationToken cancellationToken)
        {
            var fromdate = request.FromDate.Date;
            DateTime? todate = null;
            ...
            new {
                m.RefDevice,
                Year = m.DateObserved.Year,
                Month = m.DateObserved.Month
            }
            into
            gg
            select
            new MonthlyMeasureDetail()
            {
                StationId = gg.Key.RefDevice,
                Year = gg.Key.Year,
                Month = gg.Key.Month,
